macro_state.stepfull = function(macro_state, parms, time) {
		macro_states = sapply(1:parms$n_sims, function(run) {
			micro_state = lift_macro_state(macro_state)
      micro_state = micro_state_stepto(micro_state, parms, time = time, timeto = time + parms$macro_timestep, record = parms$micro_record, run = run)
...
macro_state.stepproject = function(macro_state, parms, time) {
      ...
      micro_state_relaxed = micro_state_stepto(micro_state, parms, time = time, timeto = relaxed_time, run = run, record=parms$micro_record)
      next_time = relaxed_time + parms$micro_timestep
      micro_state_next = micro_state_stepto(micro_state_relaxed, parms, time = relaxed_time, timeto = next_time, run = run, record=parms$micro_record)
