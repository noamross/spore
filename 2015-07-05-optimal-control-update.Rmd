...
shadshoot_base = function(ivals, sys, parms, times, target) {
  init = c(N=N0, P=P0, S1=ivals[1], S2=ivals[2])
  out = lsoda(init, times, sys, parms)
...
shadshoot = function(ivals, sys, parms, times, target) {
  memoizedCall(shadshoot_base, ivals=ivals, sys=sys, parms=parms, times = times, target=target)
}
...
# #
set.seed(0)
#opt0 <- nloptr_tr(x0 = c(S10,S20), eval_f = profun, lb=c(-1e6, -1e6), ub=c(1e6, 1e6),
#               opts = list(algorithm = "NLOPT_GN_CRS2_LM", maxeval=700, xtol_rel = -1, ftol_rel = -1, print_level = #ifelse(interactive(), 3, 0), ranseed = .Random.seed[1]), sys=syseq2, parms=parms, times=times)
#opt0trace = tracer(opt0)
#opt = nloptr_tr(x0 = opt0$solution, eval_f = profun, lb=c(-Inf, -Inf), ub=c(Inf, Inf),
#             opts = list(algorithm = "NLOPT_LN_BOBYQA", maxeval=400, xtol_rel = -1, ftol_rel = -1, print_level = #ifelse(interactive(), 3, 0), ranseed = .Random.seed[1]), sys=syseq2, parms=parms, times=times)

opt = nloptr_tr(x0 = c(S10, S20), eval_f = profun, lb=c(-Inf, -Inf), ub=c(Inf, Inf),
             opts = list(algorithm = "NLOPT_LN_BOBYQA", maxeval=400, xtol_rel = -1, ftol_rel = -1, print_level = ifelse(interactive(), 3, 0), ranseed = .Random.seed[1]), sys=syseq2, parms=parms, times=times)
...
set.seed(0)
opt0_s <- nloptr_tr(x0 = opt$solution, eval_f = shadshoot, lb=c(-1e6, -1e6), ub=c(1e6, 1e6),
               opts = list(algorithm = "NLOPT_GN_CRS2_LM", maxeval=1000, xtol_rel = -1, ftol_rel = -1, stopval = 0, print_level = ifelse(interactive(), 3, 0), ranseed = .Random.seed[1]), sys=syseq2, parms=parms, times=times, target = c(0,0))

opt0_s_trace = tracer(opt0_s)

opt_s = nloptr_tr(x0 = opt0_s$solution, eval_f = shadshoot, lb=c(-Inf, -Inf), ub=c(Inf, Inf),
             opts = list(algorithm = "NLOPT_LN_BOBYQA", maxeval=1000, xtol_rel = -1, ftol_rel = -1, stopval = 0, print_level = ifelse(interactive(), 3, 0), ranseed = .Random.seed[1]), sys=syseq2, parms=parms, times=times, target = c(0,0))

opttrace_s = tracer(opt_s)
opt_s
...
out = as.data.frame(out_opt)
out_tidy2 = gather(out, "variable", "value", -time)
