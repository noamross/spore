...
#    h = h_calc[which.max(H_calc)]
    ...
    return(list(derivs, c(derivs, h=h))) # h_calc = h_calc, H_calc = H_calc, H_sel=H_sel, h=h)))
  })
}
...
set.seed(0)
opt0_s <- nloptr_tr(x0 = c(0,0), eval_f = shadshoot, lb=c(0, -1e3), ub=c(1e3, 0),
...
opt_s <- nloptr_tr(x0 = next_x0, eval_f = shadshoot, lb=c(0, -1e3), ub=c(1e3, 0),
               opts = list(algorithm = "NLOPT_LN_SBPLX", maxeval=10000, xtol_abs = .Machine$double.eps , xtol_rel = -1, ftol_rel = -1, stopval = sqrt(.Machine$double.eps), print_level = ifelse(interactive(), 3, 0), ranseed = 1), sys=syseq2, parms=parms, times=times, target = c(0,0), inits=c(N0, P0))

# next_x0 = opt1_s$solution
# 
# opt_s = nloptr_tr(x0 = next_x0, eval_f = shadshoot, lb=next_x0 - 0.1, ub=next_x0 + 0.1, opts = list(algorithm = "NLOPT_GN_CRS2_LM", maxeval=-1, xtol_rel = -1, xtol_abs = -1, ftol_rel = -1 , ftol_abs = -1, stopval = sqrt(.Machine$double.eps), maxeval=1000, population = 10, print_level = ifelse(interactive(), 3, 0), ranseed = 2), sys=syseq2, parms=parms, times=times, target = c(0,0), inits=c(N0, P0))
